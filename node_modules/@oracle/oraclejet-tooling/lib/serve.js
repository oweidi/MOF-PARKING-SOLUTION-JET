...
function _validateDestination(options, platform) {
  ...
  if (platform === 'web') {
    if (destination && CONSTANTS.SUPPORTED_SERVE_WEB_DESTINATIONS.indexOf(destArray[0]) === -1) {
      throw util.toError(`Destination '${destination}' not supported. Please use '--destination=[<${CONSTANTS.SUPPORTED_SERVE_WEB_DESTINATIONS.toString()}>]`);
    }
    ...
    if (CONSTANTS.SUPPORTED_SERVE_HYBRID_DESTINATIONS.indexOf(destArray[0]) === -1) {
      throw util.toError(`Destination '${destination}' not supported. Please use '--destination=[<${CONSTANTS.SUPPORTED_SERVE_HYBRID_DESTINATIONS.toString()}>]`);
    }
...
function _validateDestinationTarget(destArray) {
  ...
  if (destArray[0] === 'browser') {
    ...
    if (supportedBrowsers.indexOf(target) === -1) {
      throw util.toError(`Browser '${target}' not supported. Please use '--browser=[<${supportedBrowsers.toString()}>]`);
    }
  } else {
...
function _applyLivereloadConditions(options) {
  ...
  if (validOptions.buildType === 'release' && validOptions.livereload === true) {
    throw util.toError('Livereload can\'t be used for release build');
  }
